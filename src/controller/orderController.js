...
const creatOrder = async function (req, res) {
  try {
    ...
    return res.status(200).send({ status: true, message: "order details", data: order })
...
const updateOrder = async function (req, res) {
  try {
    ...
    if (!["pending", "completed", "cancelled"].includes(data.status)) return res.status(400).send({ status: false, message: "status should be any of these values :- pending, completed, cancelled " });
